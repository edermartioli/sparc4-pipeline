...
def set_timecoords_keys(hdr, timezone=-3, timetype="", ra="", dec="", set_airmass=True) :
    ...
    timestr = "{}-{}-{}".format(hdr["DATE-OBS"][:4],hdr["DATE-OBS"][4:6],hdr["DATE-OBS"][6:])
    #timestr = hdr["DATE-OBS"]
...
def identify_files (p, night, print_report=True) :
            ...
            if header["OBSTYPE"] == 'OBJECT':
                ...
                if header["INSTMODE"] == 'PHOT' :
                    ...
                    if header["WPSEL"] == 'L2' :
                        ...
                        objsInPolarL4data[j][header["OBJECT"]].append(inputdata[j][i])
                        scipolar_l4[j].append(inputdata[j][i])

            elif header["OBSTYPE"] == 'FLAT':
            
                if header["OBJECT"] == 'DFLAT' or header["OBJECT"] == 'FLAT':
                    dflats[j].append(inputdata[j][i])
                elif header["OBJECT"] == 'SFLAT' :
                    sflats[j].append(inputdata[j][i])
                    
            elif header["OBSTYPE"] == 'ZERO':
    ...
    if print_report :
        ...
        for j in range(len(channels)) :
            ...
            print("\tNumber of images in channel {}: {}".format(channels[j], len(inputdata[j])))
        print("---------------------------------------")
        print("Total number of calibration images: {}".format(ndflats+nsflats+nzeros))
        print("\tNumber of zero images: {}".format(nzeros))
        if nzeros :
            for j in range(len(channels)) :
                print("\t\t {} zero images in channel {}".format(len(zeros[j]),channels[j]))

        print("\tNumber of dome flat images: {}".format(ndflats))
        if ndflats :
            for j in range(len(channels)) :
                print("\t\t {} dome flat images in channel {}".format(len(dflats[j]),channels[j]))

        print("\tNumber of sky flat images: {}".format(nsflats))
        if nsflats :
            for j in range(len(channels)) :
                print("\t\t {} sky flat images in channel {}".format(len(sflats[j]),channels[j]))

        print("---------------------------------------")
        print("Total number of science images: {}".format(nsci))
        if nsci :
            for j in range(len(channels)) :
                print("\t {} science images in channel {}".format(len(sciences[j]),channels[j]))
            
        print("\tTotal number of images in PHOT: {}".format(nsciphot))
        if nsciphot :
            for j in range(len(channels)) :
                print("\t\t {} PHOT images in channel {}".format(len(sciphot[j]),channels[j]))

        print("\tTotal number of images in POLAR: {}".format(nscipolar))
        if nscipolar :
            for j in range(len(channels)) :
                print("\t\t {} POLAR images in channel {}".format(len(scipolar[j]),channels[j]))

        print("\t\tNumber of POLAR images in L/2: {}".format(nscipolar_l2))
        if nscipolar_l2 :
            for j in range(len(channels)) :
                print("\t\t\t {} POLAR L/2 images in channel {}".format(len(scipolar_l2[j]),channels[j]))
        print("\t\tNumber of POLAR images in L/4: {}".format(nscipolar_l4))
        if nscipolar_l4 :
            for j in range(len(channels)) :
                print("\t\t\t {} POLAR L/4 images in channel {}".format(len(scipolar_l4[j]),channels[j]))
        print("---------------------------------------")
        for j in range(len(channels)) :
            print("Total number of objects observed in channel {}: {}".format(channels[j],len(objects[j])))

            for object in objects[j] :
                print("\t{} has {} images in channel {}".format(object,len(objectsdata[j][object]),channels[j]))
  
            print("\t\t {} objects observed in PHOT ".format(len(objsInPhot[j])))
            for object in objsInPhot[j] :
                print("\t\t\t{} has {} images in PHOT".format(object,len(objsInPhotdata[j][object])))

            print("\t\t {} objects observed in POLAR ".format(len(objsInPolar[j])))
            for object in objsInPolar[j] :
                print("\t\t\t{} has {} images".format(object,len(objsInPolardata[j][object])))
                
            print("\t\t {} objects observed in POLAR L/2".format(len(objsInPolarL2[j])))
            for object in objsInPolarL2[j] :
                print("\t\t\t{} has {} images".format(object,len(objsInPolarL2data[j][object])))

            print("\t\t {} objects observed in POLAR L/4".format(len(objsInPolarL4[j])))
            for object in objsInPolarL4[j] :
                print("\t\t\t{} has {} images".format(object,len(objsInPolarL4data[j][object])))
            print("---------------------------------------")
        print("***************************************")
