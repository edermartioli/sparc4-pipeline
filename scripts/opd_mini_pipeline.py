...
parser.add_option("-b", "--bias", dest="bias",
                  help="wildcard for bias selection", type='string', default="")
parser.add_option("-F", "--flat", dest="flat",
                  help="wildcard for flat selection", type='string', default="")
parser.add_option("-s", "--science", dest="science",
                  help="wildcard for science data selection", type='string',
                  default="")
parser.add_option("-o", "--object", dest="object",
                  help="object id", type='string', default="unknown_object")
parser.add_option("-t", "--time_key", dest="time_key",
                  help="time keyword", type='string', default="DATE-OBS")
parser.add_option("-a", "--ra", dest="ra", help="RA",
                  type='string', default="")
parser.add_option("-d", "--dec", dest="dec",
                  help="Dec", type='string', default="")
...
if options.plot:
    # plot master bias
    s4plt.plot_cal_frame(p["master_bias"], percentile=99.5,
                         combine_rows=True, combine_cols=True)
...
p["ASTROM_REF_IMG"] = "{}/calibdb/20230503_s4c3_CR1_astrometryRef_stack.fits".format(
    sparc4_pipeline_dir)
...
for j in range(nloops):
    ...
    print("Running loop {} of {} -> images in loop: {} to {} ... ".format(j,
          nloops, first, last))
