...
if options.checkdata :
    ...
    for channel in p['SELECTED_CHANNELS'] :
        ...
        for k in range(len(objs)) :
            ...
            detector_modes = s4db.get_detector_modes_observed(objdb, science_only=True, detector_keys=p["DETECTOR_MODE_KEYWORDS"])
            ...
            if instmodes[i][0] == "POLAR" :
                polarmodes = s4db.get_polar_modes_observed(objdb, science_only=True)
...
for channel in p['SELECTED_CHANNELS'] :
    ...
    j = channel - 1
    data_dir = p['data_directories'][j]
    dest_dir = '{}/sparc4acs{}/{}/'.format(options.destdir, p['CHANNELS'][j], options.nightdir)
    ...
    for k in range(len(in_objs)) :
        ...
        for key in detector_modes.keys() :
            ...
            biaslist = s4db.get_file_list(db, obstype=p['BIAS_OBSTYPE_KEYVALUE'], detector_mode=detector_modes[key])

            transfer_files(biaslist, dest_dir, symboliclinks=options.symboliclinks, doit=doit, force=False, printcommand=True)

            flatlist = s4db.get_file_list(db, obstype=p['FLAT_OBSTYPE_KEYVALUE'], detector_mode=detector_modes[key])
            
            transfer_files(flatlist, dest_dir, symboliclinks=options.symboliclinks, doit=doit, force=False, printcommand=True)
            ...
            for instmode in ["PHOT","POLAR"] :
                ...
                if instmodes[instmodes["INSTMODE"] == instmode] :
                    ...
                    if instmode == "POLAR" :
                        polarmodes = s4db.get_polar_modes_observed(db, science_only=True)
                        ...
                        for polarmode in ["L2","L4"] :
                            ...
                            if polarmodes[polarmodes["WPSEL"] == polarmode] :
                                print("==>>\t",obj,instmode,polarmode,detector_modes[key])
                                objlist =  s4db.get_file_list(db, object_id=obj, inst_mode=instmode, polar_mode=polarmode, obstype=p['OBJECT_OBSTYPE_KEYVALUE'], calwheel_mode=None, detector_mode=detector_modes[key])
                                # get list of objects observed
                                if len(objlist) :
                                    transfer_files(objlist, dest_dir, symboliclinks=options.symboliclinks, doit=doit, force=False, printcommand=True)
